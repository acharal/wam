...
pprInstr (ExecuteVariable, xs)      = pprInstrAux "execute_variable" (pprRegs xs)
pprInstr (Call (p,n), _)            = pprInstrAux "call" [pprPredAux p n]
pprInstr (CallVariable, xs)         = pprInstrAux "call_variable" (pprRegs xs)
...
pprInstr (PutApplication n, xs)     = pprInstrAux "put_application" $ (int n):(pprRegs xs)
